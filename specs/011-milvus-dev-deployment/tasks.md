...
- [ ] T001 同步 `grc-knowledge-engine` 的 `feature/release260831` 到最新远端基线，确认工作区干净
- [ ] T002 [P] [US1] 记录 `milvuspool` 资源、taint、现有 Pod、Namespace 和 StorageClass
  基线到 `grc-knowledge-engine/deploy/aks/milvus/README.md`
- [ ] T003 [P] [US4] 将官方 `milvus-4.2.49.tgz` vendoring 到
...
- [ ] T005 [US1] 在 values 和 smoke 工件尚未创建时运行静态测试，确认红灯
- [ ] T006 [P] [US1] 创建 `grc-knowledge-engine/deploy/aks/milvus/smoke/tests/`
  下的 pytest，先定义凭据初始化幂等性、CRUD/search、失败非零退出和 collection 清理行为
- [ ] T007 [US1] 在 smoke 实现缺失时运行 pytest，确认红灯
...
- [ ] T008 [P] [US1] 将 Chart 兼容的 `milvusdb/etcd:3.5.18-r1` 同步到项目 ACR并记录 digest
- [ ] T009 [P] [US1] 创建最小 pymilvus 2.5.x smoke 镜像并推送项目 ACR，记录 digest
- [ ] T010 [US1] 创建 `grc-knowledge-engine/deploy/aks/milvus/values-dev.yaml`，
  固定 Chart 模式、镜像 digest、资源、PVC、调度和 probe
- [ ] T011 [US1] 创建 `grc-knowledge-engine/deploy/aks/milvus/scripts/New-MilvusSecrets.ps1`，
...
- [ ] T013 [US1] 创建 smoke Dockerfile、requirements 和
  `bootstrap-auth-job.yaml`、`smoke-test-job.yaml`
- [ ] T014 [US1] 运行静态测试、smoke pytest、`helm lint`、`helm template`、
...
- [ ] T015 [US1] 创建 `milvus` Namespace 和 `milvus-credentials` Secret
- [ ] T016 [US3] 为 `milvuspool` 添加持久 taint `sku=milvus:NoSchedule`
- [ ] T017 [US1] 使用 vendored Chart执行
  `helm upgrade --install --atomic --wait --timeout 15m`
- [ ] T018 [US1] 验证 Milvus、etcd、MinIO 各一个 Pod Ready，三个 64 GiB PVC Bound，
  所有组件位于 `milvuspool`
- [ ] T019 [US1] 执行幂等认证初始化 Job，确认默认 root 密码失效、新密码可连接
- [ ] T020 [US1] 执行 CRUD/search smoke Job，确认预期主键和查询顺序，清理临时 collection/Job
...
- [ ] T021 [US2] 创建专用持久化测试 collection并记录查询断言
- [ ] T022 [US2] 删除 Milvus Standalone Pod，等待恢复 Ready并重新查询成功
- [ ] T023 [US2] 删除 etcd Pod，等待恢复 Ready并重新查询成功
- [ ] T024 [US2] 删除 MinIO Pod，等待恢复 Ready并重新查询成功
- [ ] T025 [US2] 验证三个 PVC 名称、UID、容量和 Bound 状态未变化，清理持久化测试 collection
...
- [ ] T026 [US3] 验证 Milvus、etcd、MinIO 均仅运行在 `milvuspool`
- [ ] T027 [US3] 创建不带 toleration 的普通测试 Pod并显式选择 `milvuspool`，
  确认因 untolerated taint 保持 Pending 后删除
- [ ] T028 [US3] 验证总 requests 符合 2000m CPU / 9.5 GiB 内存预算，节点及系统 Pod 健康
...
- [ ] T029 [US4] 在 `grc-knowledge-engine/deploy/aks/milvus/README.md` 编写安装、认证、
  诊断、升级、rollback、卸载、重装和显式数据清理 Runbook
- [ ] T030 [US4] 产生无害的 Helm revision 2，执行 rollback 到上一成功 revision，
  重跑查询确认数据保持
- [ ] T031 [US4] 卸载 Helm release，确认 Secret 和三个 PVC 保留，其他 Namespace 健康
- [ ] T032 [US4] 使用相同 Chart/values 重装，重新执行认证初始化与 CRUD/search smoke
- [ ] T033 [US4] 删除全部临时 Job/Pod/collection，确认 release Ready、PVC Bound、
...
- [ ] T034 [P] 更新 `docs/temp/部署镜像清单.md`，补充 Chart、兼容 etcd、smoke 镜像
  的来源、目标 digest、架构和许可证状态
- [ ] T035 [P] 更新 `docs/temp/部署服务相关任务清单.md`，记录 Milvus 拓扑、PVC、
  taint 和验收结果
- [ ] T036 运行 `grc-knowledge-engine` 适用门禁、部署专项测试、密钥扫描和 Git diff检查
- [ ] T037 提交 `grc-knowledge-engine` 部署工件并创建 PR，引用
  `specs/011-milvus-dev-deployment`
- [ ] T038 创建 `specs/011-milvus-dev-deployment/retro.md`，记录实施事实和偏差
- [ ] T039 **更新 Project Memory**：更新 knowledge-engine 服务卡、state 和必要的
  watchlist，提交后等待人审
