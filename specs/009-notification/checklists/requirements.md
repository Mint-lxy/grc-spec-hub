...
- 本次为 PRD v1.2 对齐重写（AC 格式 → spec-kit 风格，对齐 001~005 基准）；新增并收敛通知语义：FR-023（删除不通知）、FR-033（资产治理通知——中止发布与发布失败，补全 PRD §9.1 有规定但 M9-1 原十二类未列的缝隙）、FR-034/FR-035（撤销/占位，旧资产凭据更新载荷已取消）。
- 重写时发现 PRD 自身缝隙：M9-1 原十二类清单未含「中止发布」与「发布失败」治理通知（§9.1/M2-13 有规定）——已纳入 FR-033 并由 #54 一揽子裁定关闭。
- 重写后 Status 维持 Draft，待 peer review 后升 Reviewed（参照 001~005 流程）。
- 2026-08-23/24 澄清与冲突处理裁定：#54 一揽子归类——中止发布与发布失败合并为「资产治理通知」（十二→十三类），受众按阶段分层（首发前：创建者 + 草稿协作者；首发后：Owner/Deputy）；旧资产凭据更新载荷随凭据模型修订取消；逐条已读+「全部设为已读」按钮（FR-002）；不去重逐条产生；HC 失败链抑制（FR-026）。Open Questions 清零。PRD M9-1/§9.8/§9.1.1/版本说明与 004/watchlist/glossary 已同步级联回写。
