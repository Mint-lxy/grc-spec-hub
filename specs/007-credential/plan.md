...
| 服务 | 变更性质 | 说明 |
|------|----------|------|
| grc-python-sdk | 新增实现（首个版本） | Chat/Embedding/Rerank/Parser 四个 Port + OpenAI 兼容 Adapter + 凭据解析层（调用 grc-mgmt-service）+ 规范类型 |
| grc-mgmt-service | 新增端点 | `POST /mgmt/vault/inbound/resolve-model`（ADR-003/ADR-005 已定义响应形状，本次落地为正式契约） |
...
> service-map.md 中 grc-agent-service 已声明 `depends-on: grc-mgmt-service`，本次新增依赖
...
|------|------|----------------|-----------|--------|
| `contracts/openapi/grc-mgmt-service.yaml` | openapi | 新增路径 `POST /mgmt/vault/inbound/resolve-model`、`POST /mgmt/vault/inbound/report-credential-failure`（AC-13 降级机制，追加到已有文件的 vault 子域） | 否（新增路径，不改动已有的 chat 会话子域） | grc-python-sdk（通过消费本服务契约的方式对齐，非直接消费方声明——SDK 不在 service-map 登记为独立服务） |
...
| 四、服务边界 | ✅ SDK 经 HTTP 契约调用 grc-mgmt-service，不直连其数据库 |
...
│   ├── credential_resolver.py    # 新增：真正调用 grc-mgmt-service resolve-model
