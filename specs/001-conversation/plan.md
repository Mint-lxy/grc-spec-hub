...
Chat 对话，可挂载知识库检索（带引用证据），P0 不调用平台原生工具（平台原生 MCP）、不调用任何已上架资产，回答流式返回并
支持中止/重新生成；既有契约中的工具调用字段仅作兼容保留，并为 P1 Chat 调用已订阅 MCP 资产工具能力预留，不构成 P0 功能承诺。资产 Agent 透传会话（AC-15~17）沿用平台网关代理透传能力，不在本阶段
...
- grc-agent-service 收到一次"发消息"请求后，先同步调用 grc-mgmt-service 的内部接口取
  ...
  调用 LLM 网关做推理，生成完成后经内部接口把这一轮问答回写给 grc-mgmt-service 持久化。
- 该技术方案已在本地 PoC 骨架验证阶段验证过可行性（检索增强编排、SSE 流式输出、
...
在测试中用内存 fake / `httpx.MockTransport` 隔离，不依赖真实网络。
...
> knowledge-engine）与 service-map 现状一致，未新增依赖，无需改地图。
...
| 三、测试先行 | ⚠️ 核心编排逻辑（检索增强编排、流式输出、中止生成、知识库检索集成）已在本地骨架验证阶段用真实 LLM 网关联调过，但当时是"先实现后补测试"，未严格遵循红-绿；转正实现阶段将按新契约重新走测试先行（先写失败测试，覆盖新的"无状态+内部接口回写"模型），不直接照搬 PoC 测试 |
...
├── config.py                  # 配置（LLM 网关、grc-mgmt-service、grc-knowledge-engine 地址）
├── api/
│   └── chat.py                # POST .../messages、.../regenerate、.../cancel 三个端点
├── orchestrator.py            # 检索增强编排、流式生成、中止信号检查
├── services/
