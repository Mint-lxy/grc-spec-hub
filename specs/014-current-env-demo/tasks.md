...
- [ ] T004 确认演示身份方案：OIDC/Alice 优先，否则使用仅限 XAG 的隔离账号
...
- [ ] T016 为 `stage-jobs` Topic、`stage-worker` Subscription、Blob 容器和数据库初始化提供
  可审计、幂等的部署步骤
- [ ] T017 运行 Knowledge 单测、静态检查和镜像启动 smoke
- [ ] T018 构建并推送 Knowledge 镜像到 ACR，记录远端 digest，统一 README/Deployment 真相源
...
- [ ] T023 清除 Java 运行配置中的 localhost、`172.16.*`、默认 token 和占位 Secret
- [ ] T024 构建并推送 Auth/Mgmt/Gateway 镜像到 ACR，记录各自 commit 与远端 digest

## Phase 5: Portal 构建与入口

- [ ] T025 获取 Portal 后核对其 `AGENTS.md`、package manager、测试与构建约定
- [ ] T026 以 `NEXT_PUBLIC_USE_MOCK=false` 和真实 Gateway origin 构建 Portal
- [ ] T027 为 Portal 新增测试覆盖的 Dockerfile、`.dockerignore` 与 AKS runtime 清单
- [ ] T028 构建并推送 Portal 镜像到 ACR，记录 commit 与远端 digest
- [ ] T029 配置批准的内部浏览器入口，不创建默认公网 LoadBalancer

## Phase 6: Secret 与 AKS 部署

- [ ] T030 将 Auth/Mgmt/Gateway/Knowledge Secret 替换为真实值并记录批准的来源
- [ ] T031 部署 Auth，并验证登录/verify 与未认证拒绝
- [ ] T032 复核 Parser production、GPU、PVC、Docling/MinerU 与 `/readyz`
- [ ] T033 部署 Knowledge，验证数据库、队列、Blob、Milvus、Parser readiness
- [ ] T034 部署 Mgmt，验证上传会话、Blob 与 Knowledge 内部调用
- [ ] T035 部署 Gateway，验证 Auth/Mgmt 路由和失败传播
- [ ] T036 部署 Portal 与内部入口，验证浏览器无 kubectl 前置即可访问

## Phase 7: 端到端演示验收

- [ ] T037 验证 Portal 核心请求不命中 Mock，所有工作负载镜像均按 ACR digest 固定
- [ ] T038 使用固定 PDF/DOCX 完成上传、Blob、Mgmt、Knowledge、Parser、Milvus 真实构建
- [ ] T039 验证成功终态与可诊断失败；失败不得长期无界等待
- [ ] T040 AI SDK 真凭据可用时执行检索；不可用时验证明确降级
- [ ] T041 冻结最近一次通过的 manifest + digest 集合并演练入口逆序回滚

## Phase 8: 文档与记忆

- [ ] T042 更新 `docs/temp/部署服务相关任务清单.md` 和 Azure 通用部署经验
- [ ] T043 编写不含凭据的部署、验证、重置和回滚 Runbook
- [ ] T044 生成 `retro.md`
- [ ] T045 **更新 Project Memory**（state、服务卡、决策与 watchlist），提交人审
