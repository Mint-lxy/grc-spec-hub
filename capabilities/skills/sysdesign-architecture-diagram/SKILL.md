---
name: sysdesign-architecture-diagram
...
4. **Place boxes and draw arrows** following the layout/spacing rules in
   ...
   mask arrows behind semi-transparent fills; keep �?0px vertical gaps; legends
...
- Avoid SVG `<foreignObject>` (renders inconsistently in html2canvas) �?use plain
  `<rect>`/`<text>`.
- Keep it to ~8�?5 components for a high-level view; zoom into one subsystem for a
...
- Keep it to ~8�?5 components; zoom into one subsystem for a deep-dive instead of one crowded picture.
