...
| 88 | 【责任人：Azure 平台+开发】spec 014/015 开发演示后续生产化：①授予批准身份 Service Bus `listKeys` 或通过 Key Vault/CSI 注入连接串，将 Knowledge 从 local worker 切回 deployed；②PostgreSQL 管理员过渡连接改为最小权限服务账号并轮换；③Java 镜像重建为 non-root、Mgmt 补正式 readiness。VPN 路由已通过 RITM1805985 闭环；spec 015 真实上传/解析/104 chunks/104 vectors/检索已通过 | 部署生产化 | spec 014/015, `docs/temp/EC2已验证版本更新至AKS运维Runbook.md` | open |
| 89 | 【责任人：Gateway+Knowledge】XAG 演示临时开放路由回收——2026-09-13 用户为次日客户演示批准 Gateway `/open/v1/**` 跳过 JWT，入口仅限 Internal LoadBalancer/VPN。生产发布前必须补齐 Knowledge PAT/scope 校验、无/错 PAT 拒绝测试，并移除或收紧 Gateway 白名单 | 临时安全例外 | spec 015, grc-api-gateway `a715658` | open |
| 90 | 【责任人：Hub 维护者】修复契约门禁基线——2026-09-13 `check-contracts.ps1` 中 Auth/Knowledge/Mgmt 存在既有 Redocly errors，AsyncAPI CLI 依赖 `@asyncapi/studio@^1.2.0` 无可用版本，且缺 oasdiff；Mgmt 2.2 与 2.3 定向 lint 均为 6 errors/162 warnings，本轮可选 `jobId` 未增加错误 | 工程门禁 | gates/scripts/check-contracts.ps1, contracts/ | open |
