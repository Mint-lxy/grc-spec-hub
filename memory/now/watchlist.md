...
| 15 | 旧资产凭据模型可行性（委派授权）——已确认（用户确认 2026-08-21，PRD §13.10 #10）；**2026-08-24 被凭据模型修订取代**（项目组对齐：「资产级平台服务凭据」概念整体取消，一切资产调用使用订阅者个人凭据；待产出替代 ADR-0014 的正式 ADR） | 外部依赖 | spec 003/007, PRD §9.3 | closed |
...
| 50 | 【责任人：Zhang Hao】【plan 阶段输入】输出侧护栏机制基线已采纳（2026-08-24 用户裁定）：流式响应＝重叠滑动窗口检测、非流式＝全文缓冲；**命中坚持全文零字留存**——命中时终止上游并瞬时撤回全部已下发内容（含已通过窗口的前缀）、展示安全护栏命中提醒（提醒形态与文案待澄清，spec 006 Open Questions）。待 plan/压测确定窗口大小、重叠长度、检测位置、检测超时、首段延迟与长响应上限；引擎 SaaS/自建见 #1；spec 001 对应表述随分支调和时同步（main 上 Zhang 版含前缀保留口径，以本裁定为准收敛） | 架构/性能 | spec 001 FR-022/FR-023, spec 006 FR-042/FR-043, watchlist #1 | open |
