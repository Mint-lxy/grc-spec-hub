...
| 15 | 原「资产级服务凭据」方案已被 2026-08-23 用户裁定取代：Provider 仅声明凭据元数据，Consumer 在个人密钥库持有并真实验证密钥值；身份上下文仍需传递，跨文档清理见 #54 | 外部依赖 | spec 003/004/007, PRD §13.10 #10 | closed |
...
| 22 | 测评模板初始阈值已审定：安全合规 ≥ 90% 为硬门控；功能质量与性能基线 P0 仅展示不阻断（2026-08-23 用户裁定） | 基线 | spec 004 Open Questions | closed |
...
| 33 | 管理后台「准入测评模板维护」页面归 008-admin；004 仅承载发布链路中的模板消费语义（2026-08-23 用户裁定） | spec 归属 | spec 004 Open Questions, spec 008, PRD §5.2 | closed |
...
| 44 | 【BA 跟进】剩余 spec/plan 技术项：001 滑动窗口参数、检测超时、首段延迟与长响应上限；004 裁判模型算力并发；006 护栏性能基准/违禁词上限；007 SDK 凭据缓存 TTL；008 平台资源监控告警；009 通知去重/抑制间隔。003 的凭据校验深度、Agent Card/MCP 工具清单重试与 MCP 协议均已于 2026-08-23 裁定，不再列为未决 | BA 跟进 | specs/001/004/006/007/008/009 各「未决问题」或 plan 待确认节 | open |
...
| 50 | 【责任人：Zhang Hao】【plan 阶段输入】输出侧护栏机制已裁定：上游流式响应采用重叠滑动窗口检测，命中时保留此前已通过内容、终止上游及后续输出并追加「后续内容已拦截」；非流式响应采用全文检测。待 plan/压测确定窗口大小、重叠长度、检测位置、检测超时、首段延迟与长响应上限；引擎 SaaS/自建见 #1 | 架构/性能 | spec 001 FR-022/FR-023/Open Questions, watchlist #1 | open |
...
| 53 | MCP P0 协议已确认仅支持 Streamable HTTP，不兼容旧版 SSE；字段保留枚举结构供未来扩展（2026-08-23 用户选择 A） | 技术审核 | spec 003 FR-013/Open Questions | closed |
| 54 | 【跨文档同步】落实 2026-08-23 用户裁定：Provider 仅声明凭据元数据，Consumer 持有并验证个人密钥；首发成功后 Provider 才成为首位 Owner；取消密钥值轻发布及首发/测评/发布失败/中止结果通知，仅既有已发布资产成功变更时通知受影响订阅者。需同步 spec 001/002/007/009、PRD、ADR-005/凭据 ADR，并核对相关契约；同步完成前保持 open | spec/ADR/PRD 对齐 | specs/001/002/003/004/007/009, ADR-005, PRD §9.1/§9.3/M9-1 | open |
...
| 57 | 原催办已拆分：#50 的护栏处理机制已于 2026-08-23 裁定，剩余参数转为 plan 阶段输入；spec 001 进 plan 的架构前置仅剩 #45（ADR-007 是否 Accept） | 催办 | watchlist #45/#50, spec 001, ADR-007 | closed |

## 文档缺口

| # | 项目 | 类型 | 关联 | 状态 |
|---|------|------|------|------|
| 30 | API 成功响应包络（`ApiResponse`）已规范——ADR-006 定义统一包络（字符串 `code: "0"` 为成功），`error-codes.md` §1 补充成功响应格式与分页结构（2026-08-21） | 文档缺口 | architecture/adr/006-api-response-envelope.md, architecture/cross-cutting/error-codes.md | closed |
...
| 58 | `service-map.md` 未列出 grc-evaluation-service 提供 `contracts/events/eval-result.asyncapi.yaml`，与 `services.manifest.yaml` 及 mgmt-service 的 consumes 声明不一致；需同步服务地图（2026-08-23 memory-sync 检查发现） | 文档漂移 | architecture/service-map.md, architecture/services.manifest.yaml | open |
