...
| 65 | 【责任人：BA+架构】管理员/运维前台可见范围待定义——2026-09-04 用户提供截图更新：Phase 1 已确认平台管理员拥有最大权限，以便运维排障，避免只能在后台数据库查询；具体前台可见对象、文档级可见范围、操作边界及 spec/契约同步仍待确认。原 2026-08-25 会议结论「管理员不默认查看私有知识库文档级状态」待与本次阶段结论对账 | 权限模型 | spec 005, spec 008 | open |
| 66 | 【责任人：BA】知识库联系人展示范围与字段——2026-08-28 用户裁定：无权限用户对可发现知识库可在 Information 页看到 Owner 信息；「完全隐藏」知识库仍不向无权限用户暴露。2026-09-04 用户提供截图更新：知识库 Owner/Deputy 仅在创建时输入，后续从 Alice 同步；展示字段（姓名/邮箱/部门）仍未确定，其他有效权限角色是否展示同一字段集亦未定 | 产品决策 | spec 005 FR-037f, Alice | open |
...
| 69 | 【责任人：合规+BA】敏感信息局部打码 vs 护栏整段拦截的机制冲突——2026-08-25 上午会议：客户明文信息（电话/密码）严禁显示，员工姓名等内部信息是否脱敏无标准；现行护栏为整条拦截/全文零字留存（spec 006 FR-041/FR-042），与局部 PII 遮蔽需求存在逻辑冲突；会议暂缓决策，待咨询合规/董事后再定；参考 ChatGPT 实践：允许记住用户主动提供的个人信息。2026-09-04 用户提供截图更新：9.11 上线版本不涉及，由 @Tianjing 维护，保留本项 | 合规 | spec 006 FR-041/FR-042, watchlist #61 | open |
...
| 76 | 【责任人：BA+UX】优化知识库创建与配置表单——`Description` 已裁定为必填（2026-08-30 用户裁定，已回写 spec 005 FR-010）；字数限制仍为 `[待确认]`，2026-08-28 会议建议的「不少于 15 字」不作为当前基线。2026-09-04 用户提供截图更新：默认 Retrieval 检索策略已确定为知识库级、生命周期配置，仅作用于当前知识库；仍待确定 `Description` 字数限制，以及检索策略的可配置项、默认值与保存权限 | 表单/检索配置 | spec 005 FR-010/FR-038 | open |
...
| 78 | 【责任人：BA+UX】优化 Overview 页面 KPI——2026-09-04 用户提供截图更新：当前方案确定展示知识库数量、可用知识库数量、文档总数、向量数（或文档数）以及知识库就绪度（UI 展示）；指标计算口径、刷新频率、展示优先级及「向量数/文档数」最终取值仍为 `[待确认]` | KPI/交互 | spec 005, spec 008, grc-ai-portal | open |
