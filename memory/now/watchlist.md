...
| 66 | 【责任人：BA】知识库联系人展示范围与字段——2026-08-25 会议：普通用户可能需要联系人/owner 信息以便线下沟通；grill-me 裁定挂起：展示范围（可见卡片者可见？）与展示字段（仅姓名/含邮箱部门）待定 | 产品决策 | spec 005 | open |
...
| 71 | 【责任人：架构/开发】knowledge/parser 契约与 spec 005 对齐差距一揽子（2026-08-25 只读核对发现，走 contract-change 流程）：①切片四策略专属参数全缺（仅通用 chunkSize/chunkOverlap）；②向量库缺索引类型/距离算法/用量展示；③重排序缺模型选择、topN 下限 1 与 spec 10–50 不符；④语言枚举缺德语、解析器枚举含已裁定不用的 DeepDoc（2026-08-23 裁定）；⑤visibility 仍为两档 [PRIVATE, PUBLIC]，未同步三档（spec 005 FR-011，2026-08-25 用户裁定）；⑥知识库删除契约为「立即不可恢复清理」与 spec 软删除+保留期语义相反；⑦两份契约 ParserConfig 互不一致 | 契约治理 | contracts/openapi/grc-knowledge-engine.yaml, contracts/openapi/grc-parser-engine.yaml, spec 005 FR-011/FR-038/FR-042 | open |
| 75 | 【责任人：BA+UX】完善权限信息展示与申请入口——在知识库页面右上角、左侧导航栏及相关管理按钮处提供醒目的权限申请入口并统一权限信息展示；Information 页面向 Viewer 展示文档名称列表，但隐藏 Pipeline 等敏感信息；需确认各角色的可见字段与入口触发条件（来源：2026-08-28 用户提供的会议待办） | 权限交互 | spec 005, spec 008, grc-ai-portal | open |
| 76 | 【责任人：BA+UX】优化知识库创建与配置表单——确认 `Description` 是否必填及字数限制，当前「不少于 15 字」仅为建议、尚未裁定；增加仅作用于当前知识库的默认 Retrieval 检索策略，并明确可配置项、默认值与保存权限（来源：2026-08-28 用户提供的会议待办） | 表单/检索配置 | spec 005 FR-010/FR-038 | open |
| 77 | 【责任人：BA+UX+开发】调整知识库构建信息展示——移除 `Build Stage Status` 模块，改在 Information 页面静态说明 Chunk、Enhancer、Embedding 等构建流程；Pipeline 增加 Enhancer 阶段；失败时展示具体出错步骤并支持跳转至手动修复页面；需确认 Viewer 隐藏 Pipeline 的权限边界与手动修复入口权限（来源：2026-08-28 用户提供的会议待办） | 构建交互 | spec 005 FR-025/FR-026/FR-038, grc-ai-portal | open |
